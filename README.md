...
自动替换综合标题、艺术家、专辑和本地音频时长，并检查候选间距。Live、Remix、伴奏、翻唱等版本冲突、明显时长偏差或候选歧义时不会替换。“设置 → 创意工坊 → 模组设置 → SPW Lyrics”中的“手动搜索歌词”可修改关键词、限定来源、预览并应用任意候选，也可永久切回当前歌曲的本地歌词。
...
AMLL 只下载并持久化 `am-lyrics/index.jsonl`，建立元数据倒排索引和平台 ID 索引，命中后仅下载一个 TTML 文件，不遍历仓库。成功歌词缓存 30 天，手动来源和本地歌词选择永久保存；未自动匹配成功的歌曲不保存搜索或失败结果，下次播放会重新搜索。数据位于 `%LOCALAPPDATA%\SPW Lyrics`。
